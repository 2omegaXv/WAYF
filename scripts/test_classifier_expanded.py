...
MODELS = [
    {"name": "Qwen_full", "dir": "models/classifier/Qwen_full", "mode": "full"},
]

# Auto-discover available translated datasets under new_data/translated
base_translated_dir = Path(__file__).parent.parent / "new_data" / "translated"
if base_translated_dir.exists():
    DATASETS = sorted([p.name for p in base_translated_dir.iterdir() if p.is_dir()])
else:
...
def run_eval(model_dir, backbone_path, batch_size, max_length, masked, test_files):
    ...
    for file_path in test_files:
        ...
        if not file_path.exists():
            ...
            continue
        # derive label for this test file
        stem = file_path.stem
        ...
        if len(dataset) == 0:
            print(f"No examples in {file_path}, skipping")
            continue
        dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=False)
        acc, preds, labels = evaluate(model, dataloader, device)
        results[label_for_file] = {"accuracy": acc, "n": len(dataset)}
        all_preds.extend(preds)
        all_labels.extend(labels)

    overall_acc = accuracy_score(all_labels, all_preds) if all_labels else 0.0
    return {"per_file": results, "overall": overall_acc}

def print_result_table(model_name, dataset_results_masked, dataset_results_unmasked):
    print(f"\n# {model_name} classifier accuracy by test file")
    print("|test_file|unmasked_acc|masked_acc|n_unmasked|n_masked|")
    print("|:---|:---:|:---:|:---:|:---:|")
    # collect all test file labels
    labels = set()
    unl = dataset_results_unmasked.get('per_file', {}) if dataset_results_unmasked else {}
    ml = dataset_results_masked.get('per_file', {}) if dataset_results_masked else {}
    labels.update(unl.keys())
    labels.update(ml.keys())
    for lab in sorted(labels):
        u = unl.get(lab, {})
        m = ml.get(lab, {})
        u_acc = u.get('accuracy', 0.0)
        m_acc = m.get('accuracy', 0.0)
        u_n = u.get('n', 0)
        m_n = m.get('n', 0)
        print(f"|{lab}|{u_acc:.4f}|{m_acc:.4f}|{u_n}|{m_n}|")
    d_overall = dataset_results_unmasked.get('overall', 0.0) if dataset_results_unmasked else 0.0
    d_overall_mask = dataset_results_masked.get('overall', 0.0) if dataset_results_masked else 0.0
    print(f"|overall|{d_overall:.4f}|{d_overall_mask:.4f}||")
...
def main():
    ...
    for model_cfg in MODELS:
        ...
        # Load config test files (preserve order)
        ...
        cfg_test_files = cfg.get('paths', {}).get('test_file', []) if cfg else []
        test_files = []
        for p in cfg_test_files:
            ppath = Path(p)
            if not ppath.is_absolute():
                ppath = project_root / ppath
            test_files.append(ppath)

        # Unmasked
        results_unmasked = run_eval(model_cfg['dir'], backbone_path, args.batch_size, args.max_length, masked=False, test_files=test_files)
        # Masked
        results_masked = run_eval(model_cfg['dir'], backbone_path, args.batch_size, args.max_length, masked=True, test_files=test_files)
